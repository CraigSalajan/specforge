/**
 * Pure markdown ATX-heading parser. Operates on raw file content (disk truth)
 * so consumers never couple to the editor buffer.
 *
 * Lives under `electron/` as a neutral, dependency-free shared utility (mirroring
 * `electron/frontmatter/frontmatter.ts`): both the main process (the spec → canonical
 * converter, `electron/sync/spec-to-canonical.ts`) and the renderer (the outline
 * panel, via the `src/app/shared/heading-parser.ts` re-export) consume it. Keeping
 * the single source of truth here means the renderer imports *into* `electron/` —
 * the sanctioned cross-tree direction — rather than the main process reaching into
 * renderer source.
 *
